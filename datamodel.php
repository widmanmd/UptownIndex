...
function selectAllProperties()
{
	...
	if(!$stmt->execute()) {
		...
		$stmt->bind_result($pk, $address, $description,$beds, $baths, $manager, $city, $state, $zip, $propertyType, $name, $occupancy, $aptNumber, $photo, $avgMain, $avgNeig, $avgOverall);
		$propertyList = array();
		while($stmt->fetch()) {
			$property = array('id'=>$pk, 'address'=>$address, 'beds'=>$beds, 'baths'=>$baths, 'manager'=>$manager, 'city'=>$city, 'state'=>$state, 
                        ...
                        'avgMain'=>$avgMain, 'avgNeig'=>$avgNeig, 'avgOverall'=>$avgOverall);
...
function selectPropertyById($id){
    ...
    if(!$stmt->execute()) {
        ...
        $city, $state, $zipCode, $propertyType, $name, $occupancy, $apt_number, $photo, $avgMain, $avgNeig, $avgOverall);
        $stmt->fetch();
        if(isset($pk)) {
            $status = 'OK';
            $msg = 'SUCCESS';
...
function insertReview($reviewArr) {
    ...
    if(!$stmt->execute()) {
        ...
        $sql = "SELECT AVG(maintenance), AVG(neighborhood) FROM review WHERE propertyID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $propertyID);
        $stmt->execute();
        $stmt->bind_result($avgMain, $avgNeig);
        $stmt->fetch();
        $avgm = round($avgMain, 2);
        $avgn = round($avgNeig, 2);
        $overall = round(($avgMain + $avgNeig) / 2, 2);
        $stmt->close();
        
        $sql = "UPDATE property SET avgMaintenance = ?, avgNeighborhood = ?, avgOverall = ? WHERE property.pk = ?;";
        $stmt = $conn->prepare($sql);
        //if($stmt == false) {file_put_contents}
        $stmt->bind_param("dddi", $avgm, $avgn, $overall, $propertyID);
        $stmt->execute();
        $stmt->close();
    }
    ...
    if(!$stmt->execute()) {
        ...
        while($stmt->fetch()) {
			$property = array('id'=>$pk, 'address'=>$address, 'beds'=>$beds, 'baths'=>$baths, 'manager'=>$managerName, 'city'=>$city, 'state'=>$state, 
						'zip'=>$zipCode, 'propertyType'=>$propertyType, 'name'=>$name, 'occupancy'=>$occupancy, 'aptNumber'=>$apt_number, 'photo'=>$photo, 
                        'avgMain'=>$avgMain, 'avgNeig'=>$avgNeig, 'avgOverall'=>$avgOverall);
			array_push($propertyList, $property);
        }
    }
